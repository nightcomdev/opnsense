...
| `net.link.ifqmaxlen`        | 512  | 50      | Max send queue size. Default maximum length (in packets) of each interface transmit queue. When a process, kernel subsystem, or firewall rule wants to send a packet, it first places it into the interface’s output queue. Set `512` if you look for very low latency. |
...
| `net.inet.tcp.abc_l_var`         | 44    | 2       | Max cwnd increment during slow-start. When ABC is enabled, instead of growing cwnd by 1 MSS per ACK, you allow it to grow by up to `abc_l_var * MSS` per ACK (when conditions permit) so that the window can grow more rapidly on fast, low-loss links. Make sure window scaling is enabled: `net.inet.tcp.rfc1323 = 1`. Without it, the receive window may cap growth prematurely. Ensure your `net.inet.tcp.recvbuf_max` and `net.inet.tcp.sendbuf_max` are large enough so that the aggressive growth doesn’t hit buffer limits. Ensure your global socket buffer limit: `kern.ipc.maxsockbuf` supports the larger windows.  |
...
| `net.isr.defaultqlimit`       | 512  | 256      | Default netisr queue limit. Defines the maximum number of packets that each software interrupt (netisr) queue can hold before new packets are dropped. Set `512` if you look for very low latency |
