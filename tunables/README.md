...
| `net.inet.ip.redirect`           | 0     | 1       | Disable sending ICMP redirects `0`=disabled Do NOT send or accept ICMP redirects (recommended for firewalls) `1`=enabled Allow ICMP redirects (less secure). Controls whether the system accepts and sends ICMP Redirect messages. |
...
| `kern.ipc.nmbclusters`      | 200000  | 65536   | Max mbuf clusters allowed. This parameter sets (or limits) the number of mbuf clusters available to the kernel’s network subsystem. In FreeBSD, network buffers are allocated from “mbufs” and associated “clusters” (each cluster typically ~2 KiB of memory) used for packet processing (receiving, sending, buffering) in the network stack.  |
| `net.inet.tcp.recvbuf_max`	| 16777216  | 2097152 | Max automatic receive buffer (16MB), default 2MB. This sets the maximum size of the TCP socket **receive buffer** that the kernel will allow for a connection. It limits how much TCP data can be buffered on the receiving side before being processed by the application or kernel. |
| `net.inet.tcp.sendbuf_max`  | 16777216  | 2097152 | Max automatic send buffer (16MB), default 2MB. This sets the maximum size of the TCP socket send buffer that the kernel will allow for a connection. It limits how much data the kernel can buffer for sending (before the application or network “catches up”). |
| `net.inet.tcp.sendspace`    | 1048576    | 32768   | Initial send socket buffer size. This tunable defines the default TCP send buffer size (in bytes) for new TCP sockets. It is the starting value for how much data TCP is allowed to have “in flight” before requiring ACKs. Other tunables related with this `net.inet.tcp.sendbuf_auto=1`, `net.inet.tcp.sendbuf_inc` should not be to small, `net.inet.tcp.sendbuf_max` Must be >= your new sendspace, `kern.ipc.maxsockbuf` Must be >= sendbuf_max |
| `net.inet.tcp.recvspace`    | 1048576    | 65536   | Initial receive socket buffer size. Controls the default TCP receive buffer size (in bytes) for new TCP connections. This is how much incoming data TCP can hold before acknowledging and slowing the sender. If changed then you need to `recvbuf_auto`=1, `recvbuf_inc` not to small (common values: 16k–64k), `recvbuf_max` >= your new recvspace, `maxsockbuf` >= recvbuf_max |
| `net.inet.udp.recvspace`    | 524288    | 42080   | Max incoming UDP datagram space    |
| `net.inet.udp.sendspace`    | 524288 | 9216    | Max outgoing UDP datagram space    |
...
| `net.inet.tcp.recvbuf_inc`  | 131072 | 16384   | Receive buffer increment step size. This parameter sets the increment size by which the kernel will grow the TCP socket receive buffer (when “auto-tuning” is enabled) for a given connection. In other words: if receive buffer auto-growth is turned on (via `net.inet.tcp.recvbuf_auto = 1`), the system starts with a smaller buffer size and, if needed (based on congestion, RTT, throughput, etc), the buffer will step up in increments of `recvbuf_inc` until the cap of `net.inet.tcp.recvbuf_max` is reached |
| `net.inet.tcp.sendbuf_inc`	| 65536 | 8192    | Send buffer increment step size. Similar in concept to the receive side: this parameter sets the increment size by which the kernel will grow the TCP socket send buffer (when auto‐tuning is enabled via `net.inet.tcp.sendbuf_auto = 1`). It determines how aggressively the send buffer can expand for a given connection, up to the cap `net.inet.tcp.sendbuf_max` |
| `net.inet.tcp.minmss`       | 536   | 216     | Minimum TCP Maximum Segment Size     |
| `net.link.ifqmaxlen`        | 1024  | 50      | Max send queue size. Default maximum length (in packets) of each interface transmit queue. When a process, kernel subsystem, or firewall rule wants to send a packet, it first places it into the interface’s output queue. Set `512` if you look for very low latency. |
| `kern.ipc.maxsockets`	      | 65536 | 65536 | Maximum number of sockets              |
| `kern.ipc.soacceptqueue`    | 768  | 128   | Defines the maximum number of pending TCP connections that can wait in a listen backlog before being accepted by a user process (like Unbound, Web UI, or a local daemon) |
| `kern.ipc.somaxconn`        | 768  | 128   | Defines the maximum number of connections queued waiting to be accepted (backlog) by a server socket in the LISTEN state. When a server receives new connections faster than it can accept them, they are placed in this queue. If the queue is full, new connections may be rejected. Related parameters `kern.ipc.nmbclusters`, `kern.ipc.maxsockbuf`, `net.inet.ip.portrange.*`, `net.inet.tcp.msl`. |
...
| `net.inet.tcp.abc_l_var`         | 2    | 2       | Max cwnd increment during slow-start. When ABC is enabled, instead of growing cwnd by 1 MSS per ACK, you allow it to grow by up to `abc_l_var * MSS` per ACK (when conditions permit) so that the window can grow more rapidly on fast, low-loss links. Make sure window scaling is enabled: `net.inet.tcp.rfc1323 = 1`. Without it, the receive window may cap growth prematurely. Ensure your `net.inet.tcp.recvbuf_max` and `net.inet.tcp.sendbuf_max` are large enough so that the aggressive growth doesn’t hit buffer limits. Ensure your global socket buffer limit: `kern.ipc.maxsockbuf` supports the larger windows.  |
| `net.inet.tcp.initcwnd_segments` | 12    | 10      | Initial congestion window segments. You can try `44` if you not happy with speed - it's legacy `44`  |
...
| `hw.igc.max_interrupt_rate` | 15000 | 8000    | Max interrupts per second (10k). 8000 is also good and keep latency low. You can test with higher values if you have better ethernet card then Intel i226 |
